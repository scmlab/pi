...
data Waiting = Senders [(Val, Pi)] | Receivers [(Name, Pi)]
   deriving Show
       -- non-empty
...
step (Recv c x p : ps, waits, news) =
  return $ doRecv (evalExpr c) x p (ps, waits, news)
...
startE =
   Nu i (Send (eN c) (eN i) (Send (eN i) (eI 100) End)) `Par`
   Recv (eN c) x (Recv (eN x) y End)
  where i = 1
        c = 2
        x = 3
        y = 4
